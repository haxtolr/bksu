...
from robotarm_mediapipe_controller import RobotArmController
...
class 	RobotArmControl:
    def __init__(self, master):
		...
		# 시리얼 통신 관련 변수
        self.serial_thread = None # 시리얼 읽기 스레드
        self.serial_running = False # 스레드가 실행중인지 여부
        self.serial_queue = queue.Queue() # 시리얼 데이터를 저장할 큐
    ...
    def read_serial(self):
        while self.serial_running:
            if self.ser.in_waiting > 0:
                ...
                if line.endswith('d'):
                    angles = line[:-1].split(',')
                    print(angles)  # ['baseAngle', 'shoulderAngle', 'upperarmAngle', 'forearmAngle']

					# 콤보박스에 추가
                    self.saved_positions.append(line)
                    self.position_combobox['values'] = self.saved_positions

					# 라벨에 표시
                    self.label.config(text="base, shoulder, upper, forearm 을 받았습니다")
    ...
    def setup_opencv_buttons(self):
        opencv_buttons_frame = Frame(self.master)
        opencv_buttons_frame.pack()

        start_opencv_button = Button(opencv_buttons_frame, text="OpenCV Start", command=self.start_opencv)
        start_opencv_button.grid(row=0, column=0, padx=5)

        stop_opencv_button = Button(opencv_buttons_frame, text="OpenCV Stop", command=self.stop_opencv)
        stop_opencv_button.grid(row=0, column=1, padx=5)

    def start_opencv(self):
        # Serial 포트가 열려 있는지 확인하고 닫기
        if hasattr(self, 'ser') and self.ser.is_open:
            self.ser.close()
            messagebox.showinfo("시리얼 연결", "시리얼 포트를 닫고 진행합니다.")

        if self.opencv_thread is None or not self.opencv_thread.is_alive():
            self.opencv_running = True


            controller = RobotArmController(self.selected_port.get() , "115200")
            self.opencv_thread = Thread(target=controller.run)
            self.opencv_thread.start()

            print("OpenCV started")
        else:
            print("OpenCV is already running")

    def stop_opencv(self):
        self.opencv_running = False
        print("OpenCV stopped")
