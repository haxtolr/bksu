...
void loop() {
  ...
  while (Serial.available() > 0) {
    ...
    while (done == 0) {
      baseStatus = servoParallelControl(baseAngle, base, servoSpeed);
      shoulderStatus = servoParallelControl(shoulderAngle, shoulder, servoSpeed);
      upperarmStatus = servoParallelControl(upperarmAngle, upperarm, servoSpeed);
      forearmStatus = servoParallelControl(forearmAngle, forearm, servoSpeed);
