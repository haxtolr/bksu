...
# 쓰레드로 인해 억셉트가 둘로 늘어남.
client_cam = socket.socket(socket.AF_INET, socket.SOCK_STREAM)  # 비디오 커넥트용 
client_mot = socket.socket(socket.AF_INET, socket.SOCK_STREAM)  # 조이스틱 쓰레드용 억셉트
...
def camMain() :  # 함수이므로 메모리에 적재만 되어 있다가 쓰레드가 호출하면 실행됨.
