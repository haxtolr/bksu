...
hands = mp_hands.Hands(static_image_mode=False,  # 정적 이미지 모드. 기본값은 False입니다.
    max_num_hands=2,  # 동시에 감지할 수 있는 최대 손의 수. 기본값은 2입니다.
    min_detection_confidence=0.5,  # 손 감지에 대한 최소 신뢰도. 기본값은 0.5입니다.
    min_tracking_confidence=0.5)  # 손 추적에 대한 최소 신뢰도. 기본값은 0.5입니다.)
...
ser = serial.Serial('1140', 9600)

def send_to_arduino(led_brightness):
    ser.write(str(led_brightness).encode())

def main_thread():
    while True:
        distance = get_distance(thumb_x, thumb_y, index_x, index_y)
        led_brightness = int((distance / max_distance) * led_bright_max)
        threading.Thread(target=send_to_arduino, args=(led_brightness,)).start()
